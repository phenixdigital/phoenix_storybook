defmodule PhxLiveStorybook.Search do
  ...
  def handle_event("search", %{"search" => %{"input" => input}}, socket) do
    entries = Enum.filter(socket.assigns.all_entries, fn entry ->
      String.contains?(entry.name, input)
    end)
  ...
  def render(assigns) do
    ~H"""
    <div id="search-container" phx-hook="SearchHook" phx-show={JS.show(transition:  {"lsb-ease-out lsb-duration-1000", "lsb-opacity-0 lsb-scale-95", "lsb-opacity-100 lsb-scale-100"})} phx-hide={JS.hide(transition: {"lsb-ease-out lsb-duration-200", "lsb-opacity-100 lsb-scale-100", "lsb-opacity-0 lsb-scale-95"})} class="lsb lsb-hidden lsb-relative lsb-z-10 lsb-duration-300">
      ...
      <div class="lsb lsb-fixed lsb-inset-0 lsb-z-10 lsb-overflow-y-auto lsb-p-4 lsb-sm:p-6 lsb-md:p-20">
        <div class="lsb lsb-mx-auto lsb-max-w-xl lsb-transform lsb-divide-y lsb-divide-gray-100 lsb-overflow-hidden lsb-rounded-xl lsb-bg-white lsb-shadow-2xl lsb-ring-1 lsb-ring-black lsb-ring-opacity-5 lsb-transition-all">
          ...
          <.form let={f} for={:search} phx-debounce={500} id="search-form" class="lsb lsb-relative">
            ...
            <%= text_input f, :input, "phx-change": "search", "phx-target": @myself, placeholder: "Search...", class: "lsb lsb-h-12 lsb-w-full lsb-border-0 lsb-bg-transparent lsb-pl-11 lsb-pr-4 lsb-text-gray-800 lsb-placeholder-gray-400 lsb-focus:ring-0 sm:text-sm"%>
          </.form>
          ...
          <ul id="search-list" class="lsb lsb-max-h-72 lsb-scroll-py-2 lsb-overflow-y-auto lsb-py-2 lsb-text-sm lsb-text-gray-800">
            <%= for {entry, i} <- Enum.with_index(@entries, 2) do %>
              ...
              <li key={"entry-#{i}"} class="lsb lsb-cursor-default lsb-select-none lsb-px-4 lsb-py-2 focus:lsb-border-blue-400">
