defmodule PhoenixStorybook.LayoutView do
  ...
  def render_breadcrumb(socket, story_path, opts \\ []) do
    ...
    <.intersperse :let={item} enum={@breadcrumbs}>
      ...
      <span class={[
        "psb",
        @span_class,
        "psb:[&:not(:last-child)]:truncate psb:last:whitespace-nowrap"
      ]}>
        {item}
      </span>
    </.intersperse>
  ...
  def font_awesome_css_path(conn), do: storybook_setting(conn, :font_awesome_css_path)

  defp title(conn_or_socket), do: storybook_setting(conn_or_socket, :title, "Phoenix Storybook")
